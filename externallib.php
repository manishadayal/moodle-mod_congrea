...
class mod_congrea_external extends external_api
{
    ...
    public static function quiz_result($quizresult) {
        ...
        $conquizid = $DB->get_field('congrea_quiz', 'id', array('congreaid' => $cm->instance,
        'quizid' => $params['quizresult']['congreaquiz']));
    ...
    public static function poll_save_parameters() {
        return new external_function_parameters(
            array(
                ...
                'data' => new external_single_structure(
                    array(
                        'dataToSave' => new external_value(PARAM_RAW, 'poll data in encoded form'),
                        'user'    => new external_value(PARAM_INT, 'user id'),
                    )
                )
            )
    ...
    public static function poll_save($cmid, $data) {
        ...
        $obj->category = $datatosave->category; // TODO.
    ...
    public static function poll_data_retrieve($categoryid, $userid) {
        ...
        // REQUIRED.
        $params = self::validate_parameters(
            self::poll_data_retrieve_parameters(),
            array(
    ...
    public static function poll_delete($qid, $userid) {
        ...
        if ($pollcategory->courseid > 0) {
            // Course poll.
            if (!$cm = get_coursemodule_from_instance('congrea', $pollcategory->instanceid,
    ...
    public static function poll_option_drop($cmid, $polloptionid) {
        ...
        // REQUIRED.
        $params = self::validate_parameters(
            self::poll_option_drop_parameters(),
            array(
                'cmid' => $cmid,
    ...
    public static function poll_update_parameters() {
        return new external_function_parameters(
            array(
                ...
                'data' => new external_single_structure(
                    array(
                        'dataToUpdate' => new external_value(PARAM_RAW, 'poll data in encoded form'),
                        'user'    => new external_value(PARAM_INT, 'user id'),
                    )
                )
            )
    ...
    public static function poll_result_parameters() {
        return new external_function_parameters(
            array(
                ...
                'data' => new external_single_structure(
                    array(
                        'resultdata' => new external_value(PARAM_RAW, 'Result data of poll'),
                        'user'    => new external_value(PARAM_INT, 'user id'),
                    )
                )
            )
